...
def update_crew_card(filename):
    ...
    old_sig = 'export const CrewCard = ({ name, role, phone, image, index = 0 }) => {'
    new_sig = 'export const CrewCard = ({ name, role, phone, image, linkedin, instagram, email, index = 0 }) => {'
    content = content.replace(old_sig, new_sig)

    # Change the footer/socials section
    ...
    new_footer = '''<div className="flex items-center gap-3">
          {email && (
            <a href={mailto:} target="_blank" rel="noopener noreferrer" className="text-gray-400 hover:text-white transition-colors" onClick={(e) => e.stopPropagation()}>
