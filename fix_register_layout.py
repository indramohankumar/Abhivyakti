...
def update_home_register(filename):
    ...
    # We will make the text relative, highly visible stroke, and stack everything vertically!
    new_register_section = r"""{/* ?? REGISTER ?? */}
        <section id="register" className="py-20 md:py-32 px-4 relative overflow-hidden bg-[#12091f] flex flex-col items-center justify-center">
          <GoldenWheel className={bsolute top-10 left-10 w-40 h-40 opacity-20 animate-[spin_65s_linear_infinite] text-gold} />
          ...
          <div className="max-w-2xl mx-auto w-full relative z-40">
            <motion.div initial={{ opacity: 0, y: 30 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }} transition={{ duration: 0.6 }}
              className={	ext-center p-8 sm:p-12 md:p-14 rounded-3xl border relative overflow-hidden backdrop-blur-xl bg-black/60 border-white/10 shadow-[0_30px_80px_rgba(0,0,0,0.8)]}
            >
              <div className="absolute top-0 left-0 w-full h-1.5 bg-gradient-to-r from-[#ffb703] via-[#ff6b35] to-[#9b1c31]"></div>
  
              <p className="text-gold-gradient text-2xl sm:text-4xl font-extrabold mb-1 pt-2 pb-1">????? ?? ?????? ????</p>
...
target="_blank" rel="noopener noreferrer"
    ...
    if re.search(regex, content):
        ...
        with codecs.open(filename, 'w', encoding='utf-8') as f:
            f.write(content)
        print("Successfully updated register section layout!")
    else:
